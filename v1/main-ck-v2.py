...
def get_api_auth_bearer_token(username, password):
    auth_uri = "https://" + str(args.address) + ":" + str(args.port) + "/api/auth/login"
    ...
    if response.status_code == 200:
        ...
        authorization_header = response.headers['Set-Cookie']
...
def get_camera_list(bearer_token):
    bootstrap_uri = "https://" + str(args.address) + ":" + str(args.port) + "/proxy/protect/api/cameras"
    response = requests.get(bootstrap_uri, headers={'Cookie': '' + bearer_token}, verify=args.verify_ssl)
    if response.status_code == 200:
        print("Successfully retrieved data from /proxy/protect/api/cameras")
        json_response = response.json()
        print("Cameras found:")
        camera_list = []
        for camera in json_response:
...
# file downloader
def download_file(uri, file_name, bearer_token):
    global files_downloaded, files_skipped

    # make the GET request to retrieve the video file or snapshot
    try:
        response = requests.get(uri, headers={'Cookie': '' + bearer_token}, verify=args.verify_ssl, timeout=args.download_timeout)
...
def download_footage(camera_id, camera_name, bearer_token):
    ...
    for date_time_interval_start, date_time_interval_end in calculate_intervals(date_time_obj_start, date_time_obj_end):
        ...
        address = "https://" + str(args.address) + ":" + str(args.port) + "/proxy/protect/api/video/export?" + \
...
if not args.create_snapshot:
    ...
    if args.camera_ids == 'all':
        for api_camera in api_camera_list:
            download_footage(api_camera['id'], api_camera['name'], api_auth_bearer_token)
    else:
